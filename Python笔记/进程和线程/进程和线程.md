...
![](https://github.com/wushiwe-beep/wushiwe-beep.github.io/blob/main/Python%E7%AC%94%E8%AE%B0/%E8%BF%9B%E7%A8%8B%E5%92%8C%E7%BA%BF%E7%A8%8B/Picture/Untitled%201.png)
...
![](https://github.com/wushiwe-beep/wushiwe-beep.github.io/blob/main/Python%E7%AC%94%E8%AE%B0/%E8%BF%9B%E7%A8%8B%E5%92%8C%E7%BA%BF%E7%A8%8B/Picture/Untitled%202.png)
...
![](https://github.com/wushiwe-beep/wushiwe-beep.github.io/blob/main/Python%E7%AC%94%E8%AE%B0/%E8%BF%9B%E7%A8%8B%E5%92%8C%E7%BA%BF%E7%A8%8B/Picture/Untitled%203.png)


-----------------------------------------------------------
- 进程就是正在运行的程序
- 单进程就是你的电脑只能做一件事情（如皋你的电脑打开了微信就不能再打开淘宝了）
- 多进程相当于一边玩游戏一边打代码，你的电脑在同一时间里面可以“同时"执行多个任务（其实一个单核的CPU在一个时间点上只能运行一个程序，之所以能感觉到你电脑上的程序在同时运行是因为CPU在做着切换，而且切换速度超级快，快到让你感觉你开着的所有程序好像是在同时运行）
- 在一个进程里面可以执行多个任务，在这里的每一个任务就是线程，线程可以说是程序用CPU的一个基本单元。所以一个程序里面如果只是单一的一个执行路径，那么它就是单线程的。一个程序如果有多个执行路径，那么它就是多线程的。
- 并行：一个单核的CPU它每次只能执行一个进程，那么如果是多核的CPU就可以同时执行多个进程（在某一个时间段里，可以同时执行多个线程）
- 并发：在一个时间点，同时执行多个线程（双十一很多人同时秒杀一个商品）

系统里面进程和进程之间是互相独立的，而一个进程里面的多个线程，这些线程之间是共享一个进程空间的。

GIL锁，用来控制线程执行权限的，所以当一个线程需要执行的时候要先获得GIL之后才可以执行，这样就不会产生线程安全问题。

协程也叫微线程，在一个线程里面可以执行多个函数，线程和进程是通过系统调度的，而微线程则不需要，可以根据需要自己调度，因为微线程是函数之间在切换，所以开销很小

### 常用的多线程模块有：_thread  threading  Queue
